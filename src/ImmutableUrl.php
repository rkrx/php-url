...
class ImmutableUrl implements UriInterface {
	...
	public function getScheme() {
		return $this->parts[PHP_URL_SCHEME];
	}
	...
	public function getAuthority() {
		$parts = array();
		if(array_key_exists(PHP_URL_USER, $this->parts)) {
			$parts[] = urlencode($this->parts[PHP_URL_USER]);
		}
		if(array_key_exists(PHP_URL_HOST, $this->parts)) {
			$parts[] = urlencode($this->parts[PHP_URL_HOST]);
		}
		$authority = join('@', $parts);
		if($authority) {
			$parts = array($authority);
			if(array_key_exists(PHP_URL_PORT, $this->parts)) {
				$parts[] = $this->parts[PHP_URL_PORT];
			}
			$authority = join(':', $parts);
		}
		return $authority;
	...
	public function getUserInfo() {
		$parts = array();
		if(array_key_exists(PHP_URL_USER, $this->parts)) {
			$parts[] = $this->parts[PHP_URL_USER];
			if(array_key_exists(PHP_URL_PASS, $this->parts)) {
				$parts[] = $this->parts[PHP_URL_PASS];
			}
		}
		return join(':', $parts);
	}
	...
	public function getHost() {
		return (string) $this->parts[PHP_URL_HOST];
	}
	...
	public function getPath() {
		return (string) $this->parts[PHP_URL_PATH];
	}
	...
	public function getQuery() {
		return (string) $this->parts[PHP_URL_QUERY];
	}
	...
	public function getQueryData() {
		$query = (string) $this->parts[PHP_URL_QUERY];
		$data = array();
	...
	public function getFragment() {
		return (string) $this->parts[PHP_URL_FRAGMENT];
	}
	...
	public function __toString() {
		return $this->urlBuilder->build($this->parts);
	}
